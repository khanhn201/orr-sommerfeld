function [xs,umat,vmat,amat,gamma,f,U] = solve_osls_mhd_mesh(alpha, N, mesh_z, eps, rhos, mus, sigmas, st, g, Bx, Bz)
  [Ah,Bhh,Ch,Dhh,z,w] = semhat(N);
  [xs,f,Uss] = solve_steady(alpha, N, mesh_z, eps, rhos, mus, sigmas, st, g, Bx, Bz);

  Nelem = length(mesh_z) - 1;
  nh = N + 1;
  ...
  K = [
    ...
    Kau_global, -1i*alpha*Uss(end,Nelem/2);
  ];
  M = [
...
function [xs,f,Uss] = solve_steady(alpha, N, mesh_z, eps, rhos, mus, sigmas, st, g, Bx, Bz)
  [Ah,Bhh,Ch,Dhh,z,w] = semhat(N);
